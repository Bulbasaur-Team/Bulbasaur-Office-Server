    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) {
        ...
        try {
            ...
            switch (type) {
                ...
                case "retroJoin" -> retro.onJoin(session, jsonMapper.treeToValue(node, RetroJoinMessage.class));
                case "retroLeave" -> retro.onLeave(session);
                case "retroClose" -> retro.onClose(session);
                case "retroMood" -> retro.onMood(session, jsonMapper.treeToValue(node, RetroMoodMessage.class));
                case "retroAddSticker" -> retro.onAddSticker(session, jsonMapper.treeToValue(node, RetroAddStickerMessage.class));
                case "retroEditSticker" -> retro.onEditSticker(session, jsonMapper.treeToValue(node, RetroEditStickerMessage.class));
                case "retroDeleteSticker" -> retro.onDeleteSticker(session, jsonMapper.treeToValue(node, RetroDeleteStickerMessage.class));
                case "retroGroupStickers" -> retro.onGroupStickers(session, jsonMapper.treeToValue(node, RetroGroupStickersMessage.class));
